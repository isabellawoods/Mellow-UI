...
public class RenderComponents extends VanillaRenderComponents {
    ...
    /// Renders the **background** centered on a {@linkplain MultiLineLabel multi-line label}.
    /// @param component The text being rendered, in order to get the width.
    /// @param width
    /// @param x The x-position of the background.
    /// @param y The y-position of the background.
    /// @param lineHeight The height of each individual line.
    /// @param maxWidth The maximum width of each lines
    ...
    public void renderForgeBetaText(int width, int textHeight, int textColor, int alpha) {
        ...
        if (status == VersionChecker.Status.BETA || status == VersionChecker.Status.BETA_OUTDATED) {
            this.drawCenteredString(new TranslationTextComponent("forge.update.beta.1", TextFormatting.RED, TextFormatting.RESET).withStyle(TextFormatting.RED, TextFormatting.BOLD),
                    true, width / 2, textHeight, textColor | alpha);
            this.drawCenteredString(new TranslationTextComponent("forge.update.beta.2"), true, width / 2, textHeight + 10, textColor | alpha);
    ...
    private void applyScissor(@Nullable ScreenRectangle rectangle) {
        if (rectangle != null) {
            if (DebuggingFlags.DEBUG_RECTANGLE_RAVE) fill(this.stack, rectangle.left(), rectangle.top(), rectangle.right(), rectangle.bottom(), rectangle.hashCode() | 128 << 24);
