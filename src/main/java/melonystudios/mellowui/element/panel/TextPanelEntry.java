...
public class TextPanelEntry extends PanelEntry {
    ...
    @Override
    public void renderEntry(RenderComponents components, int x, int y, int width, int height) {
        ...
        switch (this.alignment) {
            case RIGHT: {
                ...
                for (FormattedCharSequence line : lines) {
                    components.drawString(line, true, width - x / 2 - font.width(line), yOffset, 0xFFFFFF);
            ...
            case CENTER: {
                int yOffset = y;
                for (FormattedCharSequence line : lines) {
                    components.drawString(line, true, x * 2 - font.width(line) / 2, yOffset, 0xFFFFFF);
                    yOffset += font.lineHeight;
                }
                break;
            }
            case LEFT: {
                int yOffset = y;
                for (FormattedCharSequence line : lines) {
                    components.drawString(line, true, x, yOffset, 0xFFFFFF);
                    yOffset += font.lineHeight;
                }
                break;
            }
        ...
        super.renderEntry(components, x, y, width, height);
    }
