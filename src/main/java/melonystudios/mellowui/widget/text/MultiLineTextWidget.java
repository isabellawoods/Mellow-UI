...
import melonystudios.mellowui.backport.MUIMultiLineLabel;
...
public class MultiLineTextWidget extends AbstractStringWidget {
    ...
    private final SingleKeyCache<CacheKey, MUIMultiLineLabel> cache;
    private boolean centered = false;

    public MultiLineTextWidget(Component text, Font font) {
        this(0, 0, text, font);
    }
    ...
    public MultiLineTextWidget(int x, int y, Component text, Font font) {
        ...
        this.cache = singleKeyCache(cache -> cache.maxRows.isPresent() ?
                MUIMultiLineLabel.create(font, cache.maxWidth, cache.maxRows.getAsInt(), cache.message) :
                MUIMultiLineLabel.create(font, cache.message, cache.maxWidth)
        );
        this.active = false;
    }
    ...
    public void renderButton(PoseStack stack, int mouseX, int mouseY, float partialTick) {
        MUIMultiLineLabel label = this.cache.getValue(this.getFreshCacheKey());
        if (this.centered) {
