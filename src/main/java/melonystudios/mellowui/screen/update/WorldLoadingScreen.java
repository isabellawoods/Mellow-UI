...
public class WorldLoadingScreen extends Screen {
    ...
    private final TrackingChunkStatusListener listener;
    ...
    public WorldLoadingScreen(TrackingChunkStatusListener listener) {
    ...
    public void render(MatrixStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        int progress = MathHelper.clamp(this.listener.getProgress(), 0, 100);
        long currentTime = Util.getMillis();
        if (currentTime - this.lastNarration > NARRATION_DELAY_MS) {
            this.lastNarration = currentTime;
            NarratorChatListener.INSTANCE.sayNow(new TranslationTextComponent("narrator.loading", progress + "%").getString());
        }
