...
/// A **panel** is essentially a {@linkplain net.minecraft.client.gui.components.AbstractSelectionList list} but focused on text and images.
@OnlyIn(Dist.CLIENT)
public class Panel extends AbstractContainerEventHandler implements Widget, NarratableEntry {
    ...
    public Panel(int x, int y, int width, int height, Screen parentScreen, Component title) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    public void renderContents(PoseStack stack, int relativeY, int mouseX, int mouseY, float partialTicks) {
    ...
    protected <T extends GuiEventListener & NarratableEntry> T addEntry(T widget) {
    ...
    public int getMaxScroll() {
        return Mth.clamp(this.getContentHeight() - this.height, 0, Integer.MAX_VALUE);
    }
